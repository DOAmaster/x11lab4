...
//look for closest jelly point
  double close_dist = 9e9;
  int close_idx;
  if(nmasses > 0) {
  for (int i=0; i < nmasses; i++) {
    double d0 = mx - mass[i].pos[0];
    double d1 = my - mass[i].pos[1];
...
if ( savex != mx || savey != my) {
  ...
  if (g.grabbed_point >= 0) {
    ...
    mass[g.grabbed_point].pos[0] = mx;
    mass[g.grabbed_point].pos[1] = my;
